...
json_file_path = os.path.join(data_dir_path, 'treasury_yield_all.json')#.format(year))

def run_historical_data(): #Only need to run the first time
    historical_rates = tr.get_treasury_historical(start_year =2022, 
                                            end_year = 2022)
    dm.create_dir()
...
def daily_refresh(): #update the latest data to the same csv file
    ...
    dm.save_s3(udpated_df,  'jtrade1-dir', 'data/us_treasury_yield', base_file, use_polars=False, delete_local=True)
    ...
    dm.save_local(df_dict, 'data', 'treasury_yield_all.json', delete_local=False)
    dm.save_s3(df_dict,  'jtrade1-dir', 'api', 'treasury_yield_all.json', use_polars=False, delete_local=True)
