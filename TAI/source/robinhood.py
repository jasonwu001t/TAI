...
class Robinhood:
        ...
        if len(earnings_json) == 0:
            ...
            for i in earnings_json:
                r = {
                    ...
                    'quarter': i['quarter'],
                    'date': i['report']['date'],
                    'actual_eps': i['eps']['actual'],
                    'estimate_eps': i['eps']['estimate']
                }
                final.append(r)
    ...
    def get_daily_close(self, ticker, interval='day', span='5year', bounds='regular', info=None):
        daily = rs.get_stock_historicals(ticker, interval=interval, span=span, bounds=bounds, info=info)
    ...
    def get_pe_rolling(self, ticker):
        ...
        daily_close = self.get_daily_close(ticker)
        result = pd.merge(left=daily_close, right=eps, how='left', on='date').fillna(method='ffill')
...
if __name__ == '__main__':
    ...
    print(f'Option expiration dates for {ticker}:', robinhood.get_option_dates(ticker))
    print('Top 100 symbols on Robinhood:', robinhood.get_robinhood_top100_symbols())
    ...
    print(f'Daily closing prices for {ticker}:', robinhood.get_daily_close(ticker))
